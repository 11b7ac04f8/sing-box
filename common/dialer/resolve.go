...
func (d *resolveDialer) DialContext(ctx context.Context, network string, destination M.Socksaddr) (net.Conn, error) {
	...
	if C.TCPConcurrent && len(addresses) > 1 {
		return dialConcurrentNetwork(ctx, d.dialer, network, destination, addresses, nil, nil, nil, d.fallbackDelay)
	}
	if d.parallel {
...
func (d *resolveDialer) ListenPacket(ctx context.Context, destination M.Socksaddr) (net.PacketConn, error) {
	...
	if C.TCPConcurrent && len(addresses) > 1 {
		conn, destinationAddress, err = listenConcurrentNetworkPacket(ctx, d.dialer, destination, addresses, nil, nil, nil, d.fallbackDelay)
	} else {
